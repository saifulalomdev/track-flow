...
export function Sidebar({ children, className }: { children: ReactNode; className?: string }) {
    ...
    return (
        <>
            ...
            <div onClick={close}
                className={cn(
                    "fixed inset-0 px-4 py-5 z-40 bg-background/80 backdrop-blur-sm transition-opacity lg:hidden",
            ...
            <aside
                className={cn(
                    // Base & Desktop Styles
                    "fixed px-4 py-5 inset-y-0 left-0 z-50 flex w-72 flex-col border-r bg-background transition-transform duration-300 ease-in-out lg:static lg:translate-x-0",
                    // Mobile Styles
...
export function SidebarHeader({ children, className }: { children?: ReactNode; className?: string }) {
    return (
        <div className={cn("flex h-16 items-center shrink-0", className)}>
            {children || (
                <Link to="/" className="transition-opacity flex items-end text-sm hover:opacity-80">
